...
const Navbar = () => {
  ...
  const links = (
    <div className={`${isScrolled || !isHome ? 'text-black flex gap-10 lg:gap-16 h-full items-center' : 'text-white flex gap-10 lg:gap-16 h-full items-center'}`}>
      <Link href={`#`}>Buy</Link>
  ...
  return (
    <nav className={`fixed w-full ${isScrolled || !isHome ? 'bg-white text-black transition-colors duration-300 z-50' : 'bg-background transition-colors duration-300 text-black z-50'}`}>
      <div className='flex max-w-screen-2xl mx-auto items-center justify-between text-white p-2 md:px-5 md:py-5'>
        <div className="flex gap-12">
          {/* Site name div */}
          <div>
            <Link href={`/`} className={`${isScrolled && 'text-black'} ${!isScrolled && !isHome && 'text-black'} 'text-white text-2xl font-semibold italic'`}>Better</Link>
          </div>
          {/* Links div */}
